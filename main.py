...
def get_stats(ticker, country, exchange, row_count):
	...
	try:
		...
		logging.error(f"Error getting stats for {ticker}")
		return

	update_stock_price(analysis.indicators["close"], row_count)
...
def update_stock_price(price, row_count):
	sheet.cell(row=row_count, column=4).value = price
...
def main():
	...
	while True:
		...
		country = sheet.cell(row=row_count, column=2).value
		exchange = sheet.cell(row=row_count, column=3).value
		if ticker is None:
			break

		# Create a new thread for each ticker symbol
		thread = threading.Thread(target=get_stats, args=[ticker, country, exchange, row_count])
		thread.start()
